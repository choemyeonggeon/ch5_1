...
public class Application02 {
	...
	public static void main(String[] args) {
		...
		/*선언한 래퍼런스 변수에 배열을 할당해서 주소값을 대입할 수있다.
		 new 연산자는 heap 영역에 공간을 생성하고 시작 위치의 주소값을 반환하는 연산자이다.
		 발생한 주소를 래퍼런스 변수(참조형 변수)에 저장하고,
		 이것을 참조하여 사용하기 떄문에 참조자료형(reference type)이라고 한다.*/
...
//		iarr = new int[];		//크기를 지정해주지 않아서 에러 발생
		...
		System.out.println("새로 할당할 배열의 길이를 입력해주세요 :" );
		int size = sc.nextInt();
		...
		//한번 할당된 배열은 지울 수 없다.
		//다만 래퍼런스 변수를 null로 변경하여 더 이상 주소를 참조할 수 없게된 배열은
		...
		System.out.println("삭게후 darr의 길이 : " + darr.length);
